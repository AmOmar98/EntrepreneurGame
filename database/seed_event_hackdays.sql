...
--   5. 10 DeliverableTemplates (T-3 polish 2026-05-10 : rubric desormais adaptee par livrable, max_score=25 uniforme).
--
...
--    Mapping ateliers -> niveaux (refondu T-3 polish 2026-05-10) :
--      M1 (L1, 13/05 09:00) Atelier Hypothese VP & Cible AgriTech (Persona + Hypothese VP)
--      M2 (L2, 13/05 11:00) Atelier BMC + MoSCoW prototype (BMC 9 blocs + MoSCoW Kanban link)
--      M3 (L3, 13/05 14:00) Atelier Concurrents + TAM/SAM/SOM (Analyse concurrentielle + sizing marche)
--      M4 (L4, 13/05 16:00) Atelier ROI/ha & Modele de portage (ROI/ha + detail CAPEX/OPEX)
...
select e.id, 'L2_solution'::public.level_id, 2, 'atelier'::public.mission_kind,
       'Atelier 2 — BMC + MoSCoW prototype',
...
select e.id, 'L3_market'::public.level_id, 3, 'atelier'::public.mission_kind,
       'Atelier 3 — Concurrents + TAM/SAM/SOM',
...
-- -----------------------------------------------------------------------------
-- 5. DeliverableTemplates — 10 livrables AgreenTech 2026 (T-3 polish 2026-05-10)
--    max_score=25 uniforme. Rubric DESORMAIS ADAPTEE PAR LIVRABLE
--    (philosophie B4 retro initiale "5 criteres uniformes" remplacee par
--    rubric ciblee selon nature pedagogique du livrable, voir BRIEF.md).
--    Slugs preserves (Option 1 idempotency) — ON CONFLICT (mission_id, slug) DO UPDATE.
--    Note vague 2 (post-pilote v0.3) : 3 bonus livrables (verbatims terrain migre,
--    development plan, prototype draft) seront heberges dans bonus_events recreate
